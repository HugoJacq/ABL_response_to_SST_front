...
USE MODI_SECOND_MNH
!
USE MODD_NSV        , ONLY : NSV_CSBEG
USE MODD_BLANK_n, ONLY : TDUMMY13 ! used for sbg w'sv' in OUT files HUGO JACQUET
!
IMPLICIT NONE
!
...
!
 ...
 END IF
  !
  IF (JSV==NSV_CSBEG) THEN ! Hugo Jacquet : save subgrid w'sv' in OUT files
  	CALL MY_UNPACK(ZFLXZ,TDUMMY13(:,:,:,1))
   ELSEIF (JSV==NSV_CSBEG+1) THEN
   	CALL MY_UNPACK(ZFLXZ,TDUMMY13(:,:,:,2))
   ELSEIF (JSV==NSV_CSBEG+2) THEN
   	CALL MY_UNPACK(ZFLXZ,TDUMMY13(:,:,:,3))
  END IF
